    ...
    distiller=dict(
        ...
        distill_losses=dict(
            loss_dkd=dict(
                type='DKDLoss',
                tau=1,
                beta=1.5,  # ResNet101 has high capacity, needs higher beta than ResNet18 (0.5 is too weak)
                loss_weight=1,
                reduction='mean')),
...
# Update pipelines with Pillow backend and bicubic interpolation for Swin Teacher
train_pipeline = [
...
train_dataloader = dict(batch_size=128, num_workers=8, dataset=dict(pipeline=train_pipeline))
val_dataloader = dict(batch_size=64, num_workers=8, dataset=dict(pipeline=test_pipeline))
test_dataloader = dict(batch_size=64, num_workers=8, dataset=dict(pipeline=test_pipeline))
