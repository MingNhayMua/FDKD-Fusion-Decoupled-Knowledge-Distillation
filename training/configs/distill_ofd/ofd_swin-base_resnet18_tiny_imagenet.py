...
# NOTE: Original OFD (OverhaulFeatureDistillation + OFDDistiller) requires
# BatchNorm layers on the teacher for margin ReLU computation.
# Since SwinTransformer uses LayerNorm (no BN), we adapt OFD to use
# SingleTeacherDistill + ConfigurableDistiller with OFDLoss at pre-logit level.
model = dict(
    _delete_=True,
    _scope_='mmrazor',
    ...
    distiller=dict(
        ...
        teacher_recorders=dict(
            ...
            fc=dict(type='ModuleOutputs', source='head.fc')),
        distill_losses=dict(
            # OFD partial L2 loss on pre-logit features
            loss_feat=dict(type='OFDLoss', loss_weight=1.0),
